...
impl AttestationEvidence {
    pub fn new(data: &[u8]) -> Result<Self> {
        ...
        let raw_report =
            String::from_utf8(rpt.to_bytes().to_vec()).expect("failed to conv to String");
        let signed_report =
            String::from_utf8(sig.to_bytes().to_vec()).expect("failed to conv to String");
        let signing_cert =
            String::from_utf8(cert.to_bytes().to_vec()).expect("failed to conv to String");
    ...
    pub fn verify_intel_signing_certificate(&self) -> Result<X509> {
        ...
        store
            ...
            .with_context(|| "Failed to verify the intel signing certificate")?;
        
        Ok(signing_x509)
    }

    pub fn verify_rsa_sig(&self, cert: X509) -> Result<()> {
        // base64 decode signature
        let sig = openssl::base64::decode_block(self.signed_report.as_str())?;

        // Extract (RSA) pub key from cert
        let pk = cert
            .public_key()
            .with_context(|| "Could not get signing cert pk")?;

        // Verify the signature
        let mut verifier = Verifier::new(MessageDigest::sha256(), &pk).unwrap();
        let success = match verifier.verify_oneshot(&sig, self.raw_report.as_bytes()) {
            Ok(b) => b,
            Err(e) => bail!(format!("Failed to verify report signature: {:?}", e)),
        };
    ...
    /// Verifies that the attestation evidence was signed by a valid Intel certificate
    pub fn verify_remote_attestation_evidence(&self) -> Result<()> {
        // Verify the certificate is valid
        let signing_cert = self.verify_intel_signing_certificate()?;

        // Verify the signature is valid
        match self.verify_rsa_sig(signing_cert) {
            Ok(()) => Ok(()),
...
pub fn epid_remote_attestation(pk_hex: &String) -> Result<AttestationEvidence> {
    ...
    let pk_bytes = match len {
        66 => {
            // Compressed SECP256K1 Key = 33B * 2 for hex = 66
        ...
        96 => {
            // BLS public key = 48B * 2 for hex = 96
...
mod tests {
    ...
    fn test_verify_bls_report() -> Result<()> {
        ...
        evidence.verify_remote_attestation_evidence().unwrap();
